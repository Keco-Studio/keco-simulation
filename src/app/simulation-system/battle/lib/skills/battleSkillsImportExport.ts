/**
 * Battle skills import validation and merge (items are Skill-shaped objects with English keys; Excel parsing in battleSkillsImportXlsx.ts).
 */
...
export interface ImportSkillFailure {
  /** Zero-based index in the source array */
  index: number;
  /** Display label (usually id) */
  label: string;
  reason: string;
  raw: unknown;
}
...
function normalizeOneSkill(item: unknown, index: number): { skill: Skill } | { label: string; reason: string } {
  if (!item || typeof item !== 'object') {
    return { label: `#${index + 1}`, reason: 'Not a valid object' };
  }
  const o = item as Record<string, unknown>;
  ...
  if (!id) {
    return { label: `#${index + 1}`, reason: 'Missing id or id is not a string' };
  }
  if (!/^[a-zA-Z0-9_]+$/.test(id)) {
    return { label: id, reason: 'id must be letters, digits, or underscore only' };
  }

  const nameRaw = o.name;
  const name = typeof nameRaw === 'string' ? nameRaw.trim() : '';
  if (!name) {
    return { label: id, reason: 'Missing name or name is empty' };
  }

  const typeRaw = o.type;
  const typeStr = typeof typeRaw === 'string' ? typeRaw.trim() : '';
  if (typeStr !== 'attack' && typeStr !== 'heal') {
    return { label: id, reason: 'type must be attack or heal' };
  }
  const type: SkillType = typeStr as SkillType;

  const power = readFiniteNumber(o.power);
  if (power === null || power < 0) {
    return { label: id, reason: 'power must be a non-negative finite number' };
  }

  const mpCost = readFiniteNumber(o.mpCost);
  if (mpCost === null || mpCost < 0) {
    return { label: id, reason: 'mpCost must be a non-negative finite number' };
  }

  const maxCooldown = readFiniteNumber(o.maxCooldown);
  if (maxCooldown === null || maxCooldown < 0) {
    return { label: id, reason: 'maxCooldown must be a non-negative finite number' };
  }
...
/**
 * Validate in file order; duplicate ids in the same file keep the first only, later rows fail.
 */
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    if (seenIds.has(r.skill.id)) {
      failures.push({
        index: i,
        label: r.skill.id,
        reason: 'Duplicate id with an earlier row in this file',
        raw: item,
      });
      continue;
    }
...
/**
 * Merge imported skills into flat rows by id: replace existing row or append.
 */
