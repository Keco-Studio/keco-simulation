...
import { showSuccessToast, showInfoToast } from '@studio/lib/utils/toast';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { useSupabase } from '@studio/lib/SupabaseContext';
import { queryKeys } from '@studio/lib/utils/queryKeys';
import { getProject, Project } from '@studio/lib/services/projectService';
import { getLibrary, Library } from '@studio/lib/services/libraryService';
import { LibraryAssetsTableAdapter } from '@studio/components/libraries/LibraryAssetsTableAdapter';
...
import { useLibraryData } from '@studio/lib/contexts/LibraryDataContext';
...
import { VersionControlSidebar } from '@studio/components/version-control/VersionControlSidebar';
import { getVersionsByLibrary } from '@studio/lib/services/versionService';
import type { LibraryVersion } from '@studio/lib/types/version';
...
import { STUDIO_LIBRARIES_HUB_PATH } from '@/lib/studioLibrariesRoutes';
...
export default function StudioLibraryPageClient() {
  ...
  useEffect(() => {
    ...
    const handleLibraryDeleted = (event: Event) => {
      ...
      if (customEvent.detail?.libraryId === libraryId) {
        ...
        if (projectId) {
          window.location.href = `${STUDIO_LIBRARIES_HUB_PATH}?projectId=${encodeURIComponent(projectId)}`;
        } else {
          window.location.href = STUDIO_LIBRARIES_HUB_PATH;
        }
      }
    };
