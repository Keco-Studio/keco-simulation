...
import type { SectionConfig, PropertyConfig } from '@studio/lib/types/libraryAssets';
import { deleteLibraryField, updateLibraryField } from '@studio/lib/services/libraryAssetsService';
...
import descriptionIcon from '@studio/assets/images/descriptionIcon.svg';
            ...
            >
              ...
              <div className={styles.confirmActions}>
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!deleteColumnConfirm.propertyId || !deleteColumnConfirm.propertyKey) {
                      showErrorToast('Missing column id, cannot delete');
                      return;
                    }
                    setDeleteColumnConfirm((prev) => ({ ...prev, loading: true }));
                    try {
                      if (scratchColumnOps) {
                        await scratchColumnOps.onDeleteColumn({
                          propertyId: deleteColumnConfirm.propertyId,
                          propertyKey: deleteColumnConfirm.propertyKey,
                        });
