...
import { updateLibraryField } from '@studio/lib/services/libraryAssetsService';
...
export function EditColumnModal({
  ...
  const formulaInputRef = useRef<InputRef>(null);
  ...
  const insertFormulaTokenAtCursor = (rawToken: string) => {
    setEditColumnModal((prev) => {
      ...
      setTimeout(() => {
        const inputRef = formulaInputRef.current;
        if (inputRef) {
          inputRef.focus();
