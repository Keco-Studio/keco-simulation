...
import {
  ...
  PropertyConfig,
  SectionConfig,
} from '@studio/lib/types/libraryAssets';
import { AssetReferenceModal } from '@studio/components/asset/AssetReferenceModal';
import { DeleteAssetModal, ClearContentsModal, DeleteRowModal } from './LibraryAssetsTableModals';
import { MediaFileUpload } from '@studio/components/media/MediaFileUpload';
import { useSupabase } from '@studio/lib/SupabaseContext';
import { KECO_SIMULATION_STUDIO_LIBRARIES_BASE } from '@studio/lib/kecoSimulationStudioLibrariesRoutes';
import { 
  type MediaFileMetadata,
  isImageFile,
  getFileIcon 
} from '@studio/lib/services/mediaFileUploadService';
import { getUserAvatarColor } from '@studio/lib/utils/avatarColors';
import { ConnectionStatusIndicator } from '@studio/components/collaboration/ConnectionStatusIndicator';
import { StackedAvatars, getFirstUserColor } from '@studio/components/collaboration/StackedAvatars';
...
import { useYjs } from '@studio/lib/contexts/YjsContext';
...
import { normalizeReferenceSelections, normalizeReferenceValueToAssetIds } from '@studio/lib/utils/referenceValue';
...
import { TableHeader, type TableHeaderProps } from './components/TableHeader';
...
import batchEditAddIcon from '@studio/assets/images/BatchEditAddIcon.svg';
import tableAssetDetailIcon from '@studio/assets/images/ProjectDescIcon.svg';
import collaborationViewNumIcon from '@studio/assets/images/collaborationViewNumIcon.svg';
import addSectionIcon from '@studio/assets/images/addProjectIcon.svg'
...
export type LibraryAssetsTableProps = {
  ...
  /** When true, table chrome treats the user as editor (no /api/projects/.../role). For local scratch tables. */
...
export function LibraryAssetsTable({
  ...
  const apiUserRole = useUserRole(routeProjectId || undefined, supabase);
  const userRole = bypassProjectRoleForUi ? 'editor' : apiUserRole;
  ...
  const sectionInputRef = useRef<InputRef>(null);
  ...
  const handlePredefineClick = () => {
    ...
    const libraryId = routeLibraryId;
    if (!projectId || !libraryId) return;
    router.push(
      `${KECO_SIMULATION_STUDIO_LIBRARIES_BASE}/library/${encodeURIComponent(libraryId)}/predefine?projectId=${encodeURIComponent(projectId)}`
    );
  };
  ...
  const handleViewAssetDetail = (row: AssetRow, e: React.MouseEvent) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      if (projectId && libraryId) {
        const u = new URL(
          `${typeof window !== 'undefined' ? window.location.origin : ''}${KECO_SIMULATION_STUDIO_LIBRARIES_BASE}/library/${libraryId}`
        );
        u.searchParams.set('projectId', projectId);
        u.searchParams.set('focusAssetId', row.id);
                    ...
                    {isRowHovered || isRowSelected ? (
                      <div className={styles.checkboxContainer}>
                        <Checkbox
                          ...
                          onChange={(e) => {
                            e.stopPropagation();
                            handleRowSelectionToggle(row.id);
                          }}
                          onClick={(e) => {
                            e.stopPropagation();
                          }}
      ...
      <AssetCardPanel
        ...
        onLibraryClick={
          routeProjectId
            ? (libraryId) =>
                router.push(
                  `${KECO_SIMULATION_STUDIO_LIBRARIES_BASE}/library/${encodeURIComponent(libraryId)}?projectId=${encodeURIComponent(routeProjectId)}`
                )
            : undefined
        }
