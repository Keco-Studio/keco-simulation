...
import searchIcon from "@studio/assets/images/searchIcon.svg";
import moveToSearchIcon from "@studio/assets/images/moveToSearch.svg";
import moveToCloseIcon from "@studio/assets/images/moveToClose.svg";
import moveToFolderIcon from "@studio/assets/images/moveToFolder.svg";
import moveToFolderDisabledIcon from "@studio/assets/images/moveToFolder2.svg";
import moveToSelectIcon from "@studio/assets/images/moveToSelect.svg";
...
import { useNavigation } from "@studio/lib/contexts/NavigationContext";
import { useSupabase } from "@studio/lib/SupabaseContext";
import { queryKeys } from "@studio/lib/utils/queryKeys";
import { normalizeSearchString } from "@studio/lib/utils/normalizeSearchString";
import { NewProjectModal } from "@studio/components/projects/NewProjectModal";
import { EditProjectModal } from "@studio/components/projects/EditProjectModal";
...
import { DuplicateLibraryModal } from "@studio/components/libraries/DuplicateLibraryModal";
import { ExportLibraryModal } from "@studio/components/libraries/ExportLibraryModal";
import { NewFolderModal } from "@studio/components/folders/NewFolderModal";
import { EditFolderModal } from "@studio/components/folders/EditFolderModal";
import { EditAssetModal } from "@studio/components/asset/EditAssetModal";
import { AddLibraryMenu } from "@studio/components/libraries/AddLibraryMenu";
import { Project, deleteProject } from "@studio/lib/services/projectService";
import { Library, deleteLibrary, moveLibraryToFolder } from "@studio/lib/services/libraryService";
import { Folder, deleteFolder } from "@studio/lib/services/folderService";
...
import { isSimulationEmbedConfigured } from "@studio/lib/simulationClientConfig";
import { deleteAsset } from "@studio/lib/services/libraryAssetsService";
import { SupabaseClient } from "@supabase/supabase-js";
import { ContextMenu } from "./ContextMenu";
import type { UserProfileDisplay } from "@studio/lib/types/user";
...
import { useUpdateEntityName } from '@studio/lib/hooks/useCacheMutations';
import { validateName } from '@studio/lib/utils/nameValidation';
...
export function Sidebar({ userProfile, onAuthRequest }: SidebarProps) {
  ...
  useEffect(() => {
    if (currentIds.projectId && projects.length > 0 && !loadingProjects) {
      ...
      if (!currentProjectExists) {
        ...
        (async () => {
          try {
            const { globalRequestCache } = await import('@studio/lib/hooks/useRequestCache');
  ...
  const onExpand = async (keys: React.Key[], info: { node: EventDataNode<DataNode> }) => {
  ...
  const handleTreeRightClick = ({ event, node }: { event: any; node: EventDataNode<DataNode> }) => {
  ...
  const handleProjectDeleteViaAPI = useCallback(
    async (projectId: string) => {
      try {
        ...
        const { globalRequestCache } = await import('@studio/lib/hooks/useRequestCache');
        const { getCurrentUserId } = await import('@studio/lib/services/authorizationService');
        try {
