...
import { listFolders, Folder } from '@studio/lib/services/folderService';
import { listLibraries, Library, getLibrariesAssetCounts } from '@studio/lib/services/libraryService';
import { AuthorizationError, getUserProjectRole } from '@studio/lib/services/authorizationService';
import projectNoFolderPreIcon from "@studio/assets/images/projectEmptyIcon_2.png";
...
import styles from '@studio/app-styles/projectPage.module.css';
import { FolderCard } from '@studio/components/folders/FolderCard';
...
import { deleteFolder } from '@studio/lib/services/folderService';
import {
  STUDIO_LIBRARIES_HUB_PATH,
  studioLibrariesFolderUrl,
  studioLibrariesHubUrl,
  studioLibrariesLibraryUrl,
} from '@/lib/studioLibrariesRoutes';

export default function CopiedStudioProjectHub() {
  ...
  const { data: project, isLoading: projectLoading, error: projectError } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!data) {
        window.location.replace(STUDIO_LIBRARIES_HUB_PATH);
        return null;
      }
      return data;
    },
        ...
        if (await confirmDeletion('Delete this library?')) {
          try {
            ...
            if (pathname.includes(libraryId)) {
              router.push(studioLibrariesHubUrl(projectId));
            }
          } catch (err: any) {
