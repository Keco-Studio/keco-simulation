...
- **`BattleEventSource`（接真实战斗，已实现）**：位于 `src/lib/progression/sources/battleEventSource.ts`（纯函数 `battleEventsToContributions`）。订阅 `BattleSession.events`，映射：`damage_applied`(玩家)→`deal_damage`、`action_executed`(cast_skill)→`cast_skill`、`battle_ended`(left_win)→`kill_enemy`。战斗结束界面「导入成长贡献」写入 `progressionStorage.battleImports`；成长模拟器可勾选「合并真实战斗贡献」与合成推演一并 `simulate()`。
